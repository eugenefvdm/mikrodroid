/**
 * Copyright (C) Snowball
 * 
 * @author Janisk
 * 
 * Adapted for Android by Johan and Eugene
 * 
 */
...
public class WriteCommand {
	...
	public static final String TAG = "WriteCommand.java"; 
	...
	/**
	 * Exception when no such command on console and clicking on debug screen
	 * @param command
	 * @return
	 */
	...
	private byte[] writeLen(String command) {
		...
		if (s.length() < 2) {
			...
			for (int j = 0; j < s.length(); j += 2) {
				ret += (char) Integer.parseInt(s.substring(j, j + 2), 16) != 0 ? (char) Integer
						.parseInt(s.substring(j, j + 2), 16) : "";
			}
		}
		return ret.getBytes();
	}
	...
	String runCommand() {
		try {
			...
			Logger.getLogger(WriteCommand.class.getName()).log(Level.SEVERE,
					null, ex);
			Log.d(TAG, "runCommand() failed");
			return "failed";
		}
	}
}